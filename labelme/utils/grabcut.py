...
def processGrabcut(qpixmap, shape): 
    global drawing, mode, ix, iy, r, elipse, bgdModel, fgdModel, mask_back, mask_fore, poly
    ...
    while True:
        ...
        if not valid:
            logger.error('Foreground has interaction with background! Please restart!')
            mask_fore = np.zeros_like(img_roi)
            mask_back = np.zeros_like(img_roi)
        ...
        # run rect grabcut algorithm
            ...
            if mask_fore.any() or mask_back.any(): 
                logger.error('INIT_WITH_RECT is not working if you already have any mask')
                continue
            logger.info('Cutting Initial foreground from the picture')

            cv2.grabCut(img_roi, mask_global, inital_rect, bgdModel, fgdModel, iteration, cv2.GC_INIT_WITH_RECT)
            mask_global_2 = np.where((mask_global==2)|(mask_global==0),0,1).astype('uint8')
            mask_global_reverse = np.where((mask_global==2)|(mask_global==0),1,0).astype('uint8')

            target = img_roi*mask_global_2[:,:,np.newaxis]
            target_reverse = img_roi*mask_global_reverse[:,:,np.newaxis]
            brightness_subtract = np.full(target_reverse.shape, (75, 75, 75), dtype=np.uint8)
            ...
            contours, hierarchy = cv2.findContours(_target_grey, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
            ...
            poly = cv2.approxPolyDP(cnt,elipse*cv2.arcLength(cnt,True),True)
            ...
            #cv2.namedWindow('output', flags=cv2.WINDOW_GUI_NORMAL)
            ...
            cv2.grabCut(img_roi,mask_global,inital_rect,bgdModel,fgdModel,iteration,cv2.GC_INIT_WITH_MASK)        
            mask_global_2 = np.where((mask_global==2)|(mask_global==0),0,1).astype('uint8')
            mask_global_reverse = np.where((mask_global==2)|(mask_global==0),1,0).astype('uint8')
            target = img_roi*mask_global_2[:,:,np.newaxis]
            target_reverse = img_roi*mask_global_reverse[:,:,np.newaxis]
            brightness_subtract = np.full(target_reverse.shape, (50, 50, 50), dtype=np.uint8)
            target_reverse = cv2.subtract(target_reverse, brightness_subtract)
            ...
            for i in range(defects.shape[0]):
                ...
                far = tuple(cnt[f][0])
                cv2.circle(result_img,far, 2,[128,255,0], 2)
            '''                 
            cv2.imshow('image', result_img)
            ...
            for point in poly:
                polygon.addPoint(QtCore.QPointF(x0 + point[0][0], y0 + point[0][1]))
            polygon.close() 
            cv2.destroyAllWindows()
            return polygon
